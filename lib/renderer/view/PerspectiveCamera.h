...
public:
	...
	PerspectiveCamera(float fov = 45.0f,float aspectRatio = 1.0f ,float znear = 0.001f, float zfar = 50.0f, float sensetivity = 0.1f) :
		Camera(sensetivity),
		fov(fov),
		znear(znear),
		zfar(zfar) {
		projection = perspective(radians(fov), aspectRatio, znear, zfar);
	}
