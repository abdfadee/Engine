...
class IBL {
    ...
    GLuint HDRToEnv(GLuint hdrTexture, GLuint width = 512, GLuint height = 512) {
        ...
        for (unsigned int i = 0; i < 6; ++i)
        {
            Shaders::EquirectangularToCubemap->setMat4("viewProjectionMatrix", viewProjectionMatrix[i]);
            glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, envCubemap, 0);
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

            Shaders::unitBox->render(Shaders::EquirectangularToCubemap);
        }
    ...
    GLuint generateIrradianceCube(GLuint envCubemap, GLuint width = 32, GLuint height = 32) {
        ...
        for (unsigned int i = 0; i < 6; ++i)
        {
            Shaders::IrradianceConvolution->setMat4("viewProjectionMatrix", viewProjectionMatrix[i]);
            glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, irradianceMap, 0);
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

            Shaders::unitBox->render(Shaders::IrradianceConvolution);
        }
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
    ...
    // --------------------------------------------------------------------------------
        ...
        for (unsigned int mip = 0; mip < maxMipLevels; ++mip)
        {
            ...
            for (unsigned int i = 0; i < 6; ++i)
            {
                Shaders::HDRPrefilter->setMat4("viewProjectionMatrix", viewProjectionMatrix[i]);
                glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, prefilterMap, mip);

                glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
                Shaders::unitBox->render(Shaders::HDRPrefilter);
            }
        }
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
    ...
    GLuint generateBRDFLUT(GLuint width = 512, GLuint height = 512) {
        ...
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        Shaders::BRDF->use();
        Shaders::screen->render(Shaders::BRDF);
...
public:
    ...
    IBL() {
        ...
        for (int i = 0; i < 6; ++i)
            viewProjectionMatrix[i] = captureProjection * captureViews[i];
    ...
    void generateMaps(Texture* hdri) {
        GLuint envCubemap = HDRToEnv(hdri->getId());
        GLuint irradianceMap = generateIrradianceCube(envCubemap);
        GLuint prefilterMap = preFilter(envCubemap);
        GLuint brdfLUTTexture = generateBRDFLUT();

        maps = new GLuint[4]{ envCubemap, irradianceMap ,prefilterMap , brdfLUTTexture };
    }
