...
// Function for Ultimate Parallax Mapping with Raymarching
...
{ 
    ...
    // get initial values
    vec2  currentTexCoords = TexCoords;
    float currentDepthMapValue = 1.0 - texture(displacmentMap, currentTexCoords).r;
    ...
    while(currentLayerDepth < currentDepthMapValue)
    {
        ...
        currentDepthMapValue = 1.0 -texture(displacmentMap, currentTexCoords).r;  
...
{		
    ...
    vec2 TexCoords = RaymarchedParallaxMapping(TexCoords,normalize(transpose(TBN) * V));
    //vec2 TexCoords = ParallaxMapping(normalize(transpose(TBN) * V));
